...
class Horde_Http_Client_Response {
    ...
    public function __construct($uri, $stream, $headers = array())
    {
        ...
        // Pop off the first header which is always the HTTP version
        // and response code.
        $http = array_shift($headers);
        if (!preg_match('/^HTTP\/(\d.\d) (\d{3})/', $http, $httpMatches)) {
            throw new Horde_Http_Client_Exception('Failed to parse HTTP response code');
        }
