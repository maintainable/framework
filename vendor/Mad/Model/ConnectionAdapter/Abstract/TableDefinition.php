 ...
 * @license    http://maintainable.com/framework-license.txt
 */
class Mad_Model_ConnectionAdapter_Abstract_TableDefinition implements ArrayAccess
{
    ...
    public function column($name, $type, $options=array())
    {
        if ($this[$name]) {
            ...
            $column = new Mad_Model_ConnectionAdapter_Abstract_ColumnDefinition(
    ...
    public function end()
    {
        $conn = Mad_Model_Base::connection(); 

        // drop previous
        if (isset($this->_options['force'])) {
            $conn->dropTable($this->getName(), $this->_options);
        }
        $temp = !empty($this->_options['temporary']) ? 'TEMPORARY'                : null;
        $opts = !empty($this->_options['options'])   ? $this->_options['options'] : null;

        $sql  = "CREATE $temp TABLE ".$this->getName()." (\n".
                  $this->toSql()."\n".
                ") $opts";
